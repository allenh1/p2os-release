## p2os (noetic) - 2.2.1-2

The packages in the `p2os` repository were released into the `noetic` distro by running `/usr/bin/bloom-release p2os --rosdistro noetic` on `Thu, 18 Jun 2020 14:32:07 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: unknown
- rosdistro version: `null`
- old version: `2.2.1-1`
- new version: `2.2.1-2`

Versions of tools used:

- bloom version: `0.9.7`
- catkin_pkg version: `0.4.20`
- rosdep version: `0.19.0`
- rosdistro version: `0.8.2`
- vcstools version: `0.1.42`


## p2os (noetic) - 2.2.1-1

The packages in the `p2os` repository were released into the `noetic` distro by running `/usr/bin/bloom-release p2os --rosdistro noetic` on `Thu, 18 Jun 2020 14:26:37 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: unknown
- rosdistro version: `null`
- old version: `null`
- new version: `2.2.1-1`

Versions of tools used:

- bloom version: `0.9.7`
- catkin_pkg version: `0.4.20`
- rosdep version: `0.19.0`
- rosdistro version: `0.8.2`
- vcstools version: `0.1.42`


## p2os (indigo) - 2.2.0-1

The packages in the `p2os` repository were released into the `indigo` distro by running `/home/allenh1/.local/bin/bloom-release --edit-track --track indigo --ros-distro indigo p2os` on `Wed, 20 Mar 2019 15:37:54 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.3-0`
- old version: `2.2.0-0`
- new version: `2.2.0-1`

Versions of tools used:

- bloom version: `0.7.2`
- catkin_pkg version: `0.4.10`
- rosdep version: `0.15.1`
- rosdistro version: `0.7.2`
- vcstools version: `0.1.40`


## p2os (indigo) - 2.2.0-0

The packages in the `p2os` repository were released into the `indigo` distro by running `/home/allenh1/.local/bin/bloom-release --track indigo --ros-distro indigo p2os` on `Sun, 17 Mar 2019 02:43:41 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.3-0`
- old version: `2.1.1-1`
- new version: `2.2.0-0`

Versions of tools used:

- bloom version: `0.7.2`
- catkin_pkg version: `0.4.10`
- rosdep version: `0.15.1`
- rosdistro version: `0.7.2`
- vcstools version: `0.1.40`


## p2os (melodic) - 2.2.0-0

The packages in the `p2os` repository were released into the `melodic` distro by running `/home/allenh1/.local/bin/bloom-release --track melodic --ros-distro melodic p2os` on `Thu, 14 Mar 2019 19:09:57 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.1.1-3`
- old version: `2.1.1-3`
- new version: `2.2.0-0`

Versions of tools used:

- bloom version: `0.7.2`
- catkin_pkg version: `0.4.10`
- rosdep version: `0.15.1`
- rosdistro version: `0.7.2`
- vcstools version: `0.1.40`


## p2os (melodic) - 2.1.1-3

The packages in the `p2os` repository were released into the `melodic` distro by running `/usr/local/bin/bloom-release --track melodic --rosdistro melodic p2os` on `Wed, 05 Sep 2018 17:21:27 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: unknown
- rosdistro version: `null`
- old version: `2.1.1-2`
- new version: `2.1.1-3`

Versions of tools used:

- bloom version: `0.6.6`
- catkin_pkg version: `0.4.8`
- rosdep version: `0.12.2`
- rosdistro version: `0.6.9`
- vcstools version: `0.1.40`


## p2os (melodic) - 2.1.1-2

The packages in the `p2os` repository were released into the `melodic` distro by running `/usr/local/bin/bloom-release --track melodic --rosdistro melodic p2os` on `Tue, 04 Sep 2018 19:55:04 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: unknown
- rosdistro version: `null`
- old version: `2.1.1-1`
- new version: `2.1.1-2`

Versions of tools used:

- bloom version: `0.6.6`
- catkin_pkg version: `0.4.8`
- rosdep version: `0.12.2`
- rosdistro version: `0.6.9`
- vcstools version: `0.1.40`


## p2os (melodic) - 2.1.1-1

The packages in the `p2os` repository were released into the `melodic` distro by running `/usr/lib/python-exec/python3.5/bloom-release p2os --track melodic --rosdistro melodic` on `Fri, 31 Aug 2018 01:49:57 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: unknown
- rosdistro version: `null`
- old version: `2.1.1-0`
- new version: `2.1.1-1`

Versions of tools used:

- bloom version: `0.6.6`
- catkin_pkg version: `0.4.8`
- rosdep version: `0.12.2`
- rosdistro version: `0.6.9`
- vcstools version: `0.1.40`


## p2os (indigo) - 2.1.1-1

The packages in the `p2os` repository were released into the `indigo` distro by running `/home/allenh1/.local/bin/bloom-release p2os --track indigo --rosdistro indigo` on `Fri, 13 Jul 2018 20:39:17 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.3-0`
- old version: `2.1.1-0`
- new version: `2.1.1-1`

Versions of tools used:

- bloom version: `0.6.6`
- catkin_pkg version: `0.4.1`
- rosdep version: `0.11.8`
- rosdistro version: `0.6.4`
- vcstools version: `0.1.40`


## p2os (indigo) - 2.1.1-0

The packages in the `p2os` repository were released into the `indigo` distro by running `/home/allenh1/.local/bin/bloom-release p2os --track indigo --rosdistro indigo` on `Fri, 13 Jul 2018 20:22:03 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.3-0`
- old version: `2.0.3-0`
- new version: `2.1.1-0`

Versions of tools used:

- bloom version: `0.6.6`
- catkin_pkg version: `0.4.1`
- rosdep version: `0.11.8`
- rosdistro version: `0.6.4`
- vcstools version: `0.1.40`


## p2os (lunar) - 2.1.1-0

The packages in the `p2os` repository were released into the `lunar` distro by running `/usr/lib/python-exec/python3.5/bloom-release p2os --track lunar --rosdistro lunar` on `Sat, 07 Jul 2018 20:13:23 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.1.0-0`
- old version: `2.1.0-0`
- new version: `2.1.1-0`

Versions of tools used:

- bloom version: `0.6.6`
- catkin_pkg version: `0.4.6`
- rosdep version: `0.12.2`
- rosdistro version: `0.6.8`
- vcstools version: `0.1.40`


## p2os (kinetic) - 2.1.1-0

The packages in the `p2os` repository were released into the `kinetic` distro by running `/usr/lib/python-exec/python3.5/bloom-release p2os --track kinetic --rosdistro kinetic` on `Wed, 04 Jul 2018 00:51:52 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.1.0-0`
- old version: `2.1.0-0`
- new version: `2.1.1-0`

Versions of tools used:

- bloom version: `0.6.6`
- catkin_pkg version: `0.4.2`
- rosdep version: `0.12.2`
- rosdistro version: `0.6.8`
- vcstools version: `0.1.40`


## p2os (melodic) - 2.1.1-0

The packages in the `p2os` repository were released into the `melodic` distro by running `/usr/lib/python-exec/python3.5/bloom-release p2os --track melodic --rosdistro melodic` on `Wed, 04 Jul 2018 00:48:57 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: unknown
- rosdistro version: `null`
- old version: `null`
- new version: `2.1.1-0`

Versions of tools used:

- bloom version: `0.6.2`
- catkin_pkg version: `0.4.2`
- rosdep version: `0.12.2`
- rosdistro version: `0.6.8`
- vcstools version: `0.1.40`


## p2os (lunar) - 2.1.0-0

The packages in the `p2os` repository were released into the `lunar` distro by running `/usr/bin/bloom-release p2os --track lunar --rosdistro lunar` on `Tue, 01 Aug 2017 23:19:56 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.7-0`
- old version: `2.0.7-0`
- new version: `2.1.0-0`

Versions of tools used:

- bloom version: `0.5.26`
- catkin_pkg version: `0.3.3`
- rosdep version: `0.11.6`
- rosdistro version: `0.6.2`
- vcstools version: `0.1.39`


## p2os (kinetic) - 2.1.0-0

The packages in the `p2os` repository were released into the `kinetic` distro by running `/usr/bin/bloom-release p2os --track kinetic --rosdistro kinetic` on `Tue, 01 Aug 2017 23:15:25 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.6-0`
- old version: `2.0.6-0`
- new version: `2.1.0-0`

Versions of tools used:

- bloom version: `0.5.26`
- catkin_pkg version: `0.3.3`
- rosdep version: `0.11.6`
- rosdistro version: `0.6.2`
- vcstools version: `0.1.39`


## p2os (jade) - 2.1.0-0

The packages in the `p2os` repository were released into the `jade` distro by running `/usr/bin/bloom-release p2os --track jade --rosdistro jade` on `Tue, 01 Aug 2017 23:10:53 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.5-0`
- old version: `2.0.5-0`
- new version: `2.1.0-0`

Versions of tools used:

- bloom version: `0.5.26`
- catkin_pkg version: `0.3.3`
- rosdep version: `0.11.6`
- rosdistro version: `0.6.2`
- vcstools version: `0.1.39`


## p2os (lunar) - 2.0.7-0

The packages in the `p2os` repository were released into the `lunar` distro by running `/usr/bin/bloom-release -r lunar -t lunar p2os` on `Wed, 31 May 2017 00:58:02 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.6-3`
- old version: `2.0.6-3`
- new version: `2.0.7-0`

Versions of tools used:

- bloom version: `0.5.26`
- catkin_pkg version: `0.3.3`
- rosdep version: `0.11.5`
- rosdistro version: `0.6.2`
- vcstools version: `0.1.39`


## p2os (lunar) - 2.0.6-3

The packages in the `p2os` repository were released into the `lunar` distro by running `/usr/bin/bloom-release -r lunar -t lunar p2os` on `Wed, 31 May 2017 00:23:27 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.6-2`
- old version: `2.0.6-2`
- new version: `2.0.6-3`

Versions of tools used:

- bloom version: `0.5.26`
- catkin_pkg version: `0.3.3`
- rosdep version: `0.11.5`
- rosdistro version: `0.6.2`
- vcstools version: `0.1.39`


## p2os (lunar) - 2.0.6-2

The packages in the `p2os` repository were released into the `lunar` distro by running `/usr/bin/bloom-release -r lunar -t lunar p2os` on `Tue, 30 May 2017 19:46:30 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.6-0`
- old version: `2.0.6-1`
- new version: `2.0.6-2`

Versions of tools used:

- bloom version: `0.5.26`
- catkin_pkg version: `0.3.3`
- rosdep version: `0.11.5`
- rosdistro version: `0.6.2`
- vcstools version: `0.1.39`


## p2os (lunar) - 2.0.6-1

The packages in the `p2os` repository were released into the `lunar` distro by running `/usr/bin/bloom-release -r lunar -t lunar p2os` on `Tue, 30 May 2017 19:40:14 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.6-0`
- old version: `2.0.6-0`
- new version: `2.0.6-1`

Versions of tools used:

- bloom version: `0.5.26`
- catkin_pkg version: `0.3.3`
- rosdep version: `0.11.5`
- rosdistro version: `0.6.2`
- vcstools version: `0.1.39`


## p2os (lunar) - 2.0.6-0

The packages in the `p2os` repository were released into the `lunar` distro by running `/usr/bin/bloom-release p2os --track lunar --rosdistro lunar` on `Tue, 23 May 2017 15:59:53 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: unknown
- rosdistro version: `null`
- old version: `null`
- new version: `2.0.6-0`

Versions of tools used:

- bloom version: `0.5.26`
- catkin_pkg version: `0.3.1`
- rosdep version: `0.11.5`
- rosdistro version: `0.6.2`
- vcstools version: `0.1.39`


## p2os (kinetic) - 2.0.6-0

The packages in the `p2os` repository were released into the `kinetic` distro by running `/usr/bin/bloom-release p2os --track kinetic --rosdistro kinetic` on `Tue, 23 May 2017 15:53:06 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.5-0`
- old version: `2.0.5-0`
- new version: `2.0.6-0`

Versions of tools used:

- bloom version: `0.5.26`
- catkin_pkg version: `0.3.1`
- rosdep version: `0.11.5`
- rosdistro version: `0.6.2`
- vcstools version: `0.1.39`


## p2os (kinetic) - 2.0.5-0

The packages in the `p2os` repository were released into the `kinetic` distro by running `/usr/bin/bloom-release p2os --track kinetic --rosdistro kinetic` on `Thu, 26 May 2016 16:11:51 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.4-0`
- old version: `2.0.4-0`
- new version: `2.0.5-0`

Versions of tools used:

- bloom version: `0.5.21`
- catkin_pkg version: `0.2.10`
- rosdep version: `0.11.4`
- rosdistro version: `0.4.7`
- vcstools version: `0.1.38`


## p2os (jade) - 2.0.5-0

The packages in the `p2os` repository were released into the `jade` distro by running `/usr/bin/bloom-release p2os --track jade --rosdistro jade` on `Thu, 26 May 2016 16:08:01 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.4-0`
- old version: `2.0.4-0`
- new version: `2.0.5-0`

Versions of tools used:

- bloom version: `0.5.21`
- catkin_pkg version: `0.2.10`
- rosdep version: `0.11.4`
- rosdistro version: `0.4.7`
- vcstools version: `0.1.38`


## p2os (kinetic) - 2.0.4-0

The packages in the `p2os` repository were released into the `kinetic` distro by running `/usr/bin/bloom-release p2os --track kinetic --rosdistro kinetic` on `Thu, 26 May 2016 12:08:24 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: unknown
- rosdistro version: `null`
- old version: `null`
- new version: `2.0.4-0`

Versions of tools used:

- bloom version: `0.5.21`
- catkin_pkg version: `0.2.10`
- rosdep version: `0.11.4`
- rosdistro version: `0.4.7`
- vcstools version: `0.1.38`


## p2os (jade) - 2.0.4-0

The packages in the `p2os` repository were released into the `jade` distro by running `/usr/bin/bloom-release p2os --track jade --rosdistro jade` on `Thu, 26 May 2016 12:02:34 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:

- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.3-0`
- old version: `2.0.3-0`
- new version: `2.0.4-0`

Versions of tools used:

- bloom version: `0.5.21`
- catkin_pkg version: `0.2.10`
- rosdep version: `0.11.4`
- rosdistro version: `0.4.7`
- vcstools version: `0.1.38`


## p2os (indigo) - 2.0.3-0

The packages in the `p2os` repository were released into the `indigo` distro by running `/usr/lib/python-exec/python2.7/bloom-release p2os --track indigo --rosdistro indigo` on `Sun, 25 Oct 2015 17:08:11 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.2-0`
- old version: `2.0.2-0`
- new version: `2.0.3-0`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.10`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.37`


## p2os (jade) - 2.0.3-0

The packages in the `p2os` repository were released into the `jade` distro by running `/usr/lib/python-exec/python2.7/bloom-release p2os --track jade --rosdistro jade` on `Sun, 25 Oct 2015 17:02:52 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.2-0`
- old version: `2.0.2-0`
- new version: `2.0.3-0`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.10`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.37`


## p2os (indigo) - 2.0.2-0

The packages in the `p2os` repository were released into the `indigo` distro by running `/usr/lib/python-exec/python2.7/bloom-release --rosdistro indigo --track indigo p2os --edit` on `Tue, 04 Aug 2015 11:50:46 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `1.0.11-3`
- old version: `1.0.11-3`
- new version: `2.0.2-0`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.9`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (jade) - 2.0.2-0

The packages in the `p2os` repository were released into the `jade` distro by running `/usr/lib/python-exec/python2.7/bloom-release p2os --track jade --rosdistro jade` on `Tue, 04 Aug 2015 11:34:08 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.1-0`
- old version: `2.0.1-0`
- new version: `2.0.2-0`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.9`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (jade) - 2.0.1-0

The packages in the `p2os` repository were released into the `jade` distro by running `/usr/lib/python-exec/python2.7/bloom-release p2os --track jade --rosdistro jade` on `Sat, 11 Jul 2015 07:25:36 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `2.0.0-0`
- old version: `2.0.0-0`
- new version: `2.0.1-0`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.9`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (indigo) - 1.0.11-3

The packages in the `p2os` repository were released into the `indigo` distro by running `/usr/lib/python-exec/python2.7/bloom-release p2os --track indigo --rosdistro indigo` on `Sat, 11 Jul 2015 07:19:09 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `1.0.11-2`
- old version: `1.0.11-2`
- new version: `1.0.11-3`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.9`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (hydro) - 2.0.0-0

The packages in the `p2os` repository were released into the `hydro` distro by running `/usr/bin/bloom-release p2os --track hydro --ros-distro hydro` on `Tue, 05 May 2015 03:18:28 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `1.0.10-0`
- old version: `1.0.10-0`
- new version: `2.0.0-0`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.8`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (groovy) - 2.0.0-1

The packages in the `p2os` repository were released into the `groovy` distro by running `/usr/bin/bloom-release p2os --track groovy --rosdistro groovy` on `Sat, 02 May 2015 23:31:36 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `1.0.9-0`
- old version: `2.0.0-0`
- new version: `2.0.0-1`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.8`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (indigo) - 1.0.11-2

The packages in the `p2os` repository were released into the `indigo` distro by running `/usr/bin/bloom-release p2os --track indigo --rosdistro indigo` on `Sat, 02 May 2015 23:24:24 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `1.0.11-0`
- old version: `1.0.11-1`
- new version: `1.0.11-2`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.8`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (indigo) - 1.0.11-1

The packages in the `p2os` repository were released into the `indigo` distro by running `/usr/bin/bloom-release p2os --track indigo --rosdistro indigo` on `Sat, 02 May 2015 23:15:24 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `1.0.11-0`
- old version: `1.0.11-0`
- new version: `1.0.11-1`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.8`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (groovy) - 2.0.0-0

The packages in the `p2os` repository were released into the `groovy` distro by running `/usr/bin/bloom-release p2os --track groovy --rosdistro groovy` on `Sat, 02 May 2015 23:08:44 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `1.0.9-0`
- old version: `1.0.10-0`
- new version: `2.0.0-0`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.8`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (groovy) - 1.0.10-0

The packages in the `p2os` repository were released into the `groovy` distro by running `/usr/bin/bloom-release p2os --track groovy --rosdistro groovy` on `Sat, 02 May 2015 23:01:42 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: https://github.com/allenh1/p2os-release.git
- rosdistro version: `1.0.9-0`
- old version: `1.0.9-0`
- new version: `1.0.10-0`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.8`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (jade) - 2.0.0-0

The packages in the `p2os` repository were released into the `jade` distro by running `/usr/bin/bloom-release p2os --track jade --rosdistro jade` on `Sat, 02 May 2015 22:57:30 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- upstream repository: https://github.com/allenh1/p2os
- release repository: unknown
- rosdistro version: `null`
- old version: `null`
- new version: `2.0.0-0`

Versions of tools used:
- bloom version: `0.5.20`
- catkin_pkg version: `0.2.8`
- rosdep version: `0.11.2`
- rosdistro version: `0.4.2`
- vcstools version: `0.1.36`


## p2os (indigo) - 1.0.11-0

The packages in the `p2os` repository were released into the `indigo` distro by running `/usr/bin/bloom-release --ros-distro indigo --track indigo --new-track p2os` on `Wed, 25 Jun 2014 16:05:18 -0000`

These packages were released:
- `p2os_doc`
- `p2os_driver`
- `p2os_launch`
- `p2os_msgs`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `null`
- old version: `1.0.10-0`
- new version: `1.0.11-0`

Versions of tools used:
- bloom version: `0.5.10`
- catkin_pkg version: `0.2.2`
- rosdep version: `0.10.27`
- rosdistro version: `0.3.5`
- vcstools version: `0.1.35`


## p2os (indigo) - 1.0.10-0

The packages in the `p2os` repository were released into the `indigo` distro by running `/usr/bin/bloom-release --rosdistro indigo --track indigo p2os --new-track` on `Wed, 25 Jun 2014 15:20:39 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `null`
- old version: `null`
- new version: `1.0.10-0`

Versions of tools used:
- bloom version: `0.5.10`
- catkin_pkg version: `0.2.2`
- rosdep version: `0.10.27`
- rosdistro version: `0.3.5`
- vcstools version: `0.1.35`


## p2os (hydro) - 1.0.10-0

The packages in the `p2os` repository were released into the `hydro` distro by running `/usr/bin/bloom-release p2os --track hydro --rosdistro hydro` on `Wed, 28 May 2014 14:43:00 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.9-1`
- old version: `1.0.9-1`
- new version: `1.0.10-0`

Versions of tools used:
- bloom version: `0.5.9`
- catkin_pkg version: `0.2.2`
- rosdep version: `0.10.27`
- rosdistro version: `0.3.5`
- vcstools version: `0.1.35`


## p2os - 1.0.9-0

User `allenh1@allenh1-Aspire-1830T` released the packages in the `p2os` repository by running `/usr/bin/bloom-release p2os --track groovy --rosdistro groovy` on `Wed, 21 Aug 2013 13:53:04 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.7-0`
- old version: `1.0.7-0`
- new version: `1.0.9-0`

Versions of tools used:
- bloom version: `0.4.4`
- catkin_pkg version: `0.1.19`
- rosdep version: `0.10.21`
- rosdistro version: `0.2.13`
- vcstools version: `0.1.31`


## p2os - 1.0.9-1

User `allenh1@allenh1-Aspire-1830T` released the packages in the `p2os` repository by running `/usr/bin/bloom-release p2os --track hydro --rosdistro hydro` on `Sun, 18 Aug 2013 20:42:42 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.6-0`
- old version: `1.0.9-0`
- new version: `1.0.9-1`

Versions of tools used:
- bloom version: `0.4.4`
- catkin_pkg version: `0.1.19`
- rosdep version: `0.10.21`
- rosdistro version: `0.2.13`
- vcstools version: `0.1.31`


## p2os - 1.0.9-0

User `allenh1@allenh1-Aspire-1830T` released the packages in the `p2os` repository by running `/usr/bin/bloom-release p2os --track hydro --rosdistro hydro` on `Sun, 18 Aug 2013 20:39:12 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.6-0`
- old version: `1.0.6-0`
- new version: `1.0.9-0`

Versions of tools used:
- bloom version: `0.4.4`
- catkin_pkg version: `0.1.19`
- rosdep version: `0.10.21`
- rosdistro version: `0.2.13`
- vcstools version: `0.1.31`


## p2os - 1.0.7-0

User `allenh1@allenh1-Vostro-430` released the packages in the `p2os` repository by running `/usr/bin/bloom-release --rosdistro groovy --track groovy p2os --edit` on `Sat, 03 Aug 2013 14:52:46 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.6-0`
- old version: `1.0.6-0`
- new version: `1.0.7-0`

Versions of tools used:
- bloom version: `0.4.4`
- catkin_pkg version: `0.1.19`
- rosdep version: `0.10.21`
- rosdistro version: `0.2.13`
- vcstools version: `0.1.31`


## p2os - 1.0.6-0

User `allenh1@allenh1-Vostro-430` released the packages in the `p2os` repository by running `/usr/bin/bloom-release p2os --track groovy --rosdistro groovy` on `Sat, 27 Jul 2013 21:46:55 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.5-0`
- old version: `1.0.5-1`
- new version: `1.0.6-0`

Versions of tools used:
- bloom version: `0.4.4`
- catkin_pkg version: `0.1.19`
- rosdep version: `0.10.21`
- rosdistro version: `0.2.13`
- vcstools version: `0.1.31`


## p2os - 1.0.6-0

User `allenh1@allenh1-Vostro-430` released the packages in the `p2os` repository by running `/usr/bin/bloom-release p2os --track hydro --rosdistro hydro` on `Sat, 27 Jul 2013 21:44:23 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.5-0`
- old version: `1.0.5-0`
- new version: `1.0.6-0`

Versions of tools used:
- bloom version: `0.4.4`
- catkin_pkg version: `0.1.19`
- rosdep version: `0.10.21`
- rosdistro version: `0.2.13`
- vcstools version: `0.1.31`


## p2os - 1.0.5-1

User `allenh1@allenh1-Vostro-430` released the packages in the `p2os` repository by running `/usr/bin/bloom-release p2os --track groovy --rosdistro groovy` on `Wed, 24 Jul 2013 15:10:04 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.5-0`
- old version: `1.0.5-0`
- new version: `1.0.5-1`

Versions of tools used:
- bloom version: `0.4.4`
- catkin_pkg version: `0.1.19`
- rosdep version: `0.10.21`
- rosdistro version: `0.2.13`
- vcstools version: `0.1.31`


## p2os - 1.0.5-0

User `allenh1@allenh1-Vostro-430` released the packages in the `p2os` repository by running `/usr/bin/bloom-release --rosdistro groovy --track groovy p2os --edit` on `Tue, 23 Jul 2013 21:37:16 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.4-0`
- old version: `null`
- new version: `1.0.5-0`

Versions of tools used:
- bloom version: `0.4.4`
- catkin_pkg version: `0.1.19`
- rosdep version: `0.10.21`
- rosdistro version: `0.2.13`
- vcstools version: `0.1.31`


## p2os - 1.0.5-0

User `allenh1@allenh1-Vostro-430` released the packages in the `p2os` repository by running `/usr/bin/bloom-release --rosdistro hydro --track hydro p2os --edit` on `Tue, 23 Jul 2013 21:34:49 -0000`

These packages were released:
- `p2os_driver`
- `p2os_launch`
- `p2os_teleop`
- `p2os_urdf`

Version of package(s) in repository `p2os`:
- rosdistro version: `1.0.4-1`
- old version: `null`
- new version: `1.0.5-0`

Versions of tools used:
- bloom version: `0.4.4`
- catkin_pkg version: `0.1.19`
- rosdep version: `0.10.21`
- rosdistro version: `0.2.13`
- vcstools version: `0.1.31`


p2os-release
============
